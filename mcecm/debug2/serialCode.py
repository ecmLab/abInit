        ...
        for qy_idx in range(N):
            for qz_idx in range(N):
                ...
                elastic_energy += np.real(
                    np.einsum(
                        "ij,ijkl,kl->", strain_ft[qx_idx, qy_idx, qz_idx], B[qx_idx, qy_idx, qz_idx], strain_ft[qx_idx, qy_idx, qz_idx].conj(),
                    )
                )
# Normalize the elastic energy by assuming the lattice constant is a0 = 0.1nm; This is not an accurate assumption but the paper is not clear with the correct parameters.
    elastic_energy = elastic_energy / (8*pi**3)

    return elastic_energy

# Monte Carlo step with dimensionless strain
def monte_carlo_step(lattice_spins, temperature, q_grid, B, strain_ft, epsilon_1, epsilon_2, epsilon_3, current_energy):
    N = lattice_spins.shape[0]
...
def main():
    ...
    prev_energy = compute_elastic_energy(lattice_spins, q_grid, B, strain_ft)
    ...
    for step in range(1, num_steps + 1):
        ...
        total_energy = compute_elastic_energy(lattice_spins, q_grid, B, strain_ft)
