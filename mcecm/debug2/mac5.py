...
def monte_carlo_step(lattice_spins, temperature, q_grid, B, strain_field, epsilon_1,
                        epsilon_2, epsilon_3, rank, size, comm, current_energy):
    N = lattice_spins.shape[0]
...
#        print('XXX',x,y,z,current_spin, proposed_spin, E_new, current_energy, delta_E, prob)
        ...
        # Accept the proposed spin
            ...
            strain_field = strain_field_new  # Update the strain field
            current_energy = E_new
#            print(lattice_spins.reshape(27), current_energy)
            accepted = True
        else:
            accepted = False
    else:
...
# Broadcast the acceptance status and updated `lattice_spins`
    ...
    return accepted, current_energy, strain_field  # Rejected: return current energy
...
# Broadcast results to all ranks
    ...
    stiffness_tensor = comm.bcast(stiffness_tensor, root=0)
...
# Compute initial energy
    ...
    for step in range(1, num_steps + 1):
        ...
        for _ in range(lattice_spins.size // size):
            accepted, current_energy, strain_field = monte_carlo_step(lattice_spins, temperature, q_grid, B, strain_field,
