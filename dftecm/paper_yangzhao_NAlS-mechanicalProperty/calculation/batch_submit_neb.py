"""
Submit all NEB jobs for the three diffusivity phases.
Run from calculation/:  python batch_submit_neb.py
"""
...
for phase in PHASES:
    job_dir = CALC_DIR / phase / "neb" / "hop1"
    if not (job_dir / "INCAR").exists():
        skipped.append(str(job_dir))
        print(f"SKIP {phase}/neb/hop1  (no INCAR)")
        continue
